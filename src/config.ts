...
export function getRateLimitConfig(): RateLimitConfig {
    return {
        ipWindowMs: positiveInteger('WA_RATE_LIMIT_IP_WINDOW_MS', 60_000),
        ipMax: positiveInteger('WA_RATE_LIMIT_IP_MAX', 30),
        globalWindowMs: positiveInteger('WA_RATE_LIMIT_GLOBAL_WINDOW_MS', 60_000),
        globalMax: positiveInteger('WA_RATE_LIMIT_GLOBAL_MAX', 200),
        operationalWindowMs: positiveInteger('WA_RATE_LIMIT_OPERATIONAL_WINDOW_MS', 300_000),
        operationalMax: positiveInteger('WA_RATE_LIMIT_OPERATIONAL_MAX', 5),
    };
}

export function getAuthConfig(): AuthConfig {
    const rawKeys = process.env.WA_API_KEYS?.trim() ?? '';
    const rawOpKeys = process.env.WA_OPERATIONAL_API_KEYS?.trim() ?? '';
    const apiKeys = rawKeys ? rawKeys.split(',').map((k) => k.trim()).filter(Boolean) : [];
    const operationalKeys = rawOpKeys ? rawOpKeys.split(',').map((k) => k.trim()).filter(Boolean) : [];
    return { apiKeys, operationalKeys };
}

export function getQueueConfig(): QueueConfig {
    return {
        maxConcurrent: positiveInteger('WA_QUEUE_MAX_CONCURRENT', 10),
        maxQueueSize: positiveInteger('WA_QUEUE_MAX_SIZE', 50),
        timeoutMs: positiveInteger('WA_QUEUE_TIMEOUT_MS', 30_000),
    };
}
